...
func TestShouldAutoContinueClaudeToolCall(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "misspelled progress narration from Claude Code", content: "Emiting the checks now.", want: true},
		{name: "progress narration after status sentence", content: "Deploy is in its local gate phase. Monitoring until it finishes.", want: true},
		{name: "background deployment progress", content: "Deployment is running in the background.", want: true},
		{name: "background deployment with explicit follow-up", content: "Deployment is running in the background. Let me check its progress.", want: true},
		...
		{name: "completed deployment", content: "The deployment completed successfully.", want: false},
